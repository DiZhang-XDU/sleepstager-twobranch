...
class XY_dataset_N2One(data.Dataset):
    def __init__(self, tvt = 'train', serial_len = 5, frame_len = 3000, channel_num = 5, datasetName = 'SHHS1', 
        ...
        if not redir_cache:
            cache_path = './prepared_data/{:}_{:}_cache.pkl'.format(tvt, cohortName)
        else:
            cache_path = join(redir_cache, '{:}_{:}_cache.pkl'.format(tvt, cohortName))
        #if cache
        ...
        # subject selector
        if not redir_root:
            self.root = [r'E:\data\filtered_data\subjects', r'F:\data\filtered_data\subjects']
        else:
            self.root = redir_root
        subj_paths = []
        ...
        for dataset in datasetName:
            for i in range(*subjIdx[dataset]):
